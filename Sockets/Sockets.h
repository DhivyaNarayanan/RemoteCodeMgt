...
public:
	...
	Socket(::SOCKET);
	Socket(Socket&& s);  //move ctor
	operator ::SOCKET() { return socket_; }
	Socket& operator=(Socket&& s);   //move assignment operator
	virtual ~Socket();
...
bool SocketListener::start(CallObj& co)
{
	...
	std::thread ListenThread(
		[&]()
	{
		//ApplicationHelpers::Verbose::show("server waiting for connection");
		...
		std::cout << "Listen thread stopping..\n";
	}
	);
	ListenThread.join();
